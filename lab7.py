...
def mol_form(compound_formula):
    ...
    compound = {}
    index = 0
    while index < len(compound_formula):
        element = compound_formula[index]
        if (index + 1) < len(compound_formula) and compound_formula[index + 1].islower():
            element += compound_formula[index + 1]
            index += 1
        if element not in compound:
            compound[element] = 0
        if (index + 1) < len(compound_formula) and compound_formula[index + 1].isdigit():
            number = ''
            while (index + 1) < len(compound_formula) and compound_formula[index + 1].isdigit():
                    number += compound_formula[index + 1]
                    index += 1
            compound[element] += int(number)
        else:
            compound[element] += 1
        index += 1
    return compound
...
# PART 2 - Complete the function below that takes two
 #number = ''
 #while compound_formula[index + 1].isdigit():
    #number += compound_formula[index + 1]
    #index += 1
    #compound[element] = int(number)
...
def expr_form(expr_coeffs,expr_molecs):
    ...
    """
    # TODO your code here
    tuplelen = len(expr_coeffs)
   
    dictionary = {}
    for x in range(tuplelen):
        elemco = expr_coeffs[x]
        molecules = expr_molecs[x]
        for chem, num in molecules.items():
            if chem not in dictionary:
                dictionary[chem] = elemco * num
            else:
                dictionary[chem] += elemco * num
...
def find_unbalanced_atoms(reactant_atoms, product_atoms):
    ...
    for chem in reactant_atoms.keys():
       
        if (chem not in product_atoms) or (reactant_atoms[chem] != product_atoms[chem]):
            notbalanced.add(chem)

   
    for chem in product_atoms.keys():
      
        if (chem not in reactant_atoms) or (product_atoms[chem] != reactant_atoms[chem]):
            notbalanced.add(chem)
...
def check_eqn_balance(reactants,products):
    ...
    reac = expr_form(reactants[0], reac)
    prod = expr_form(products[0], prod)

    chemunb = find_unbalanced_atoms(reac, prod)
